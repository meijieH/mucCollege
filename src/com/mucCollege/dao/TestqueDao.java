...
public class TestqueDao {
	...
	// 根据Block查询
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public ArrayList<Testque> QueryTestqueByBlock(Integer blockid) {
		Session s = factory.getCurrentSession();
		String hql = "From Testque testque where 1=1";
		hql = hql + " and testque.block.blockid =" + blockid;
		Query q = s.createQuery(hql);
		List testqueList = q.list();
		return (ArrayList<Testque>) testqueList;
	}
	...
	// 根据题库中的题查
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public ArrayList<Testque> QueryTestqueByQuestion(Integer questionid) {
		Session s = factory.getCurrentSession();
		String hql = "From Testque testque where 1=1";
		hql = hql + " and testque.question.questionid=" + questionid;
		Query q = s.createQuery(hql);
		List testqueList = q.list();
		return (ArrayList<Testque>) testqueList;
	}
